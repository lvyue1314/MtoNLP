...
| 指标 | Gemma 4 | LLaVA | 基线 | 说明 |
|------|---------|-------|------|------|
| **Contains Match**（主指标） | **74.7%** | 11.2% | 6.2% | GT 完整出现在回答中，适合长回答模型 |
| Exact Match | 0.0% | 0.1% | 0.3% | 预测==GT，对长回答模型失效 |
| F1 Score | 8.4% | 1.8% | 0.3% | 词级重叠，受回答长度稀释 |
| **精确数值匹配**（辅助） | **61.9%** | 2.8% | 0.4% | GT 所有数值全部命中，最硬核 |

> **论文建议**：以 CM 为主指标、精确数值匹配为辅助指标。EM 和 F1 因多模态模型自然生成长回答而绝对值偏低，不适宜作为主指标。详见 `理解/三指标深度分析.md`。

### 四类错误自动分类（修正版）

> **重要修正**：旧版用精确文本匹配判断"失败"，导致 Gemma4 冗长回答几乎全被误判。现改用 Contains Match 过滤，只对 CM 真正失败的案例分类。

| 错误类型 | 定义 | Gemma 4 | LLaVA | 基线 |
|----------|------|---------|-------|------|
| **文字识别错误** | 编辑距离 ≤ 2 且长度相近 | 0 | 0 | 1226 |
| **视觉定位失败** | 预测关键词在图中OCR文本中存在但不在GT中 | 49 | 177 | 0 |
| **推理错误** | 共享数值但最终答案不一致 | 83 | 305 | 193 |
| **模型幻觉** | 预测数字/实体在图中OCR文本中不存在 | **503** | **1738** | 925 |
| **CM 失败总数** | — | **635** | **2220** | **2344** |

**关键发现**：
- Gemma 4 仅失败 635 题（25.4%），其中 79% 是幻觉——编造了图表中不存在的数值
- LLaVA 失败 2220 题（88.8%），78% 是幻觉——ViT→Projector 管道在数值传递上系统性失败
- 两个多模态模型的主要问题都是数值幻觉，印证了融合机制差异在精确数值读取上的决定性影响
